...
// 	      treeStep: 100,
// 	      matrixLength: 20,
// 				matrixBox: 0, // matrix块状布局内的 box大小
//
// 	      layoutFlag: 0,
// 	      verticalFlag: 0,
// 	      horizontalFlag: 0,
...
//
// 	setParams (params) {
// 		if(typeof params === 'object') { // 只针对单层浅拷贝
// 			let hasOwnProperty = Object.prototype.hasOwnProperty
// 			for (let i = 0; i < arguments.length; i++) {
// 				let other = arguments[i]
// 				for (let prop in other) {
// 					if (hasOwnProperty.call(other, prop))
// 						this.options[prop] = other[prop]
// 					}
// 				}
// 			}
//
...
// 		let connectedComp = this.connectedComp
// 		let options = this.options
// 		let levelStep = options.levelStep
//
// 		connectedComp.forEach(function(item) {
...
//       item.levelStep = (levelStep!== 0) ? levelStep: (options.height - options.nodesRadius) / item.levelTree.length - options.nodesRadius * 2
// 			// item.levelStep = 200
// 		})
// 	}
//
...
// 		let doubleRadius = radius << 1
// 		let childStep = this.options.childStep
// 		let matrixLength = this.options.matrixLength
// 	  let matrixBox = this.options.matrixBox || doubleRadius
// 		this.options.matrixBox = Math.max(matrixBox, doubleRadius)
// 		matrixBox = this.options.matrixBox
//
// 		let nodes = this.nodes
// 		let connectedComp = this.connectedComp
// 		console.log('connectedComp', connectedComp)
//
// 		for (let len = connectedComp.length, i = 0; i < len; i++) {
...
//
// 			console.log('levelTree', levelTree)
// 			console.log('levelStep', levelStep)
//       // 设置根节点
...
// 				let currentParent = levelTree[j].values[0].parent // 当前节点的父节点
// 				let currentIndex = 0
// 				let currentParentX = currentParent.x // 父节点的 x position
// 				let currentChildLength = currentParent.values.length // 当前层的孩子节点总数
// 				console.log('pre level length', levelTree[j - 1].values.length)
// 				let maxChildrenLen = this.findChildrenLenMax(levelTree[j - 1].values)
// 				console.log('maxChildrenLen', maxChildrenLen)
// 				let maxLevel = (maxChildrenLen % matrixLength) ? (Math.floor(maxChildrenLen / matrixLength)) : (Math.floor(maxChildrenLen / matrixLength) - 1)
// 				console.log('maxLevel', maxLevel)  // 树中节点层次
// 				// 保证层与层之间的最小的距离为leveStep
// 				//             1
// 				//     2       3       4
// 				//                    13 14
// 				// 5 6 7 8     9      10 11 12
//         // yStart = min(levelStep) + max(ChildrenLen)
// 				let yEnd = levelStep * (levelTree[j].key - 1)
// 				let yStart = yEnd + matrixBox * maxLevel + doubleRadius * (maxLevel + 1)
...
// 										x = parseFloat((currentIndex - 1) % 20 - 21 / 2) * (matrixBox +　0) + currentParentX
// 		                y = yStart
// 		                if(currentIndex % matrixLength !== 0) {
// 		                    // y -= Math.floor(currentIndex / matrixLength) * radius * 2
// 												y -= Math.floor(currentIndex / matrixLength) * (matrixBox +　0)
// 		                } else {
// 		                    // y -= (Math.floor(currentIndex / matrixLength) - 1) * radius * 2
// 												y -= (Math.floor(currentIndex / matrixLength) - 1) * (matrixBox +　0)
// 		                }
// 		            }
...
class ForestMatrix {
  ...
  linksFormat () {
    ...
    originLinks.forEach(function (item) {
      ...
      switch (typeof source) {
        case 'object':
          sIndex = nodes.findIndex(function (value, index, arr) {
            return value.identify == source.identify
          })
          tIndex = nodes.findIndex(function (value, index, arr) {
            return value.identify == target.identify
          })
          break
  ...
  createNeighborMatrix () {
    ...
    mylinks.forEach(function (item, i) {
      ...
      if (source !== target) {
        nodes[source].matrix.push({
          neighbor: target,
          linkIndex: i
        })
        nodes[target].matrix.push({
          neighbor: source,
          linkIndex: i
        })
      }
        ...
        for (let valuesLen = levelTree[j].values.length, k = 0; k < valuesLen; k++) {
          ...
          if (currentParent.values.length <= matrixLength) {
            ...
            y = yStart
              ...
              levelTree[j].values[k].cluster = true
              x = parseFloat((currentIndex - 1) % 20 - 21 / 2) * matrixBox + currentParentX
              y = yStart
              if (currentIndex % matrixLength !== 0) {
                	y -= Math.floor(currentIndex / matrixLength) * matrixBox
              } else {
                  y -= (Math.floor(currentIndex / matrixLength) - 1) * matrixBox
              }
          }
          levelTree[j].values[k].x = x
          levelTree[j].values[k].y = y
  ...
  adjustCoverNodes (connectedComp, node, moveDistX) {
    ...
    while (parentIndex) {
      ...
      if (!parent.cluster) {
        ...
        let index = levelTree[parent.depth - 1].values.findIndex(function (d) {
    ...
    while (currentDepth <= levelLen - 1) {
      let key
      let value
      let levelIndex = levelTree[currentDepth].values.findIndex(function (value, index, arr) {
        return treeArray.indexOf(value.parent) >= currentIndex
      })
      if (levelIndex != -1) {
        value = treeArray.indexOf(levelTree[currentDepth].values[levelIndex])
        key = currentDepth + 1
